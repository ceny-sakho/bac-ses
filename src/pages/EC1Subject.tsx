...
const EC1Subject: React.FC = () => {
  ...
  const pdfUrl = `/ec1/chapitre${chapter}/sujet-${sujetNumber}.pdf`;

  useEffect(() => {
    document.title = `EC1 Chapitre ${chapter} - Sujet ${sujetNumber} | PDF`;
  }, [chapter, sujetNumber]);

  const handleDownload = () => {
    const link = document.createElement('a');
    link.href = pdfUrl;
    link.download = `ec1-chapitre${chapter}-sujet-${sujetNumber}.pdf`;
  ...
  return (
    <main className="max-w-7xl mx-auto px-4 py-8">
      <Button
        variant="ghost"
        onClick={() => navigate(`/ec1/${chapter}`, { replace: true })}
      ...
      <h1 className="text-2xl font-bold mb-6">
        EC1 Chapitre {chapter} — Sujet {sujetNumber}
      </h1>
