...
const EC3Subject: React.FC = () => {
  ...
  const pdfUrl = `/ec3/chapitre${chapter}/sujet-${sujetNumber}.pdf`;

  useEffect(() => {
    document.title = `EC3 Chapitre ${chapter} - Sujet ${sujetNumber} | PDF`;
  }, [chapter, sujetNumber]);

  const handleDownload = () => {
    const link = document.createElement('a');
    link.href = pdfUrl;
    link.download = `ec3-chapitre${chapter}-sujet-${sujetNumber}.pdf`;
  ...
  return (
    <main className="max-w-7xl mx-auto px-4 py-8">
      <Button
        variant="ghost"
        onClick={() => navigate(`/ec3/${chapter}`, { replace: true })}
      ...
      <h1 className="text-2xl font-bold mb-6">
        EC3 Chapitre {chapter} — Sujet {sujetNumber}
      </h1>
